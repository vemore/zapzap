...
clean `HEAD` on the dev machine, tags them `<short sha>` and `latest`, pushes them to the LAN
...
`scripts/deploy_nas.sh --rollback <sha>` redeploys an earlier deploy's images. Configuration:
