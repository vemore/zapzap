...
- **The Rust backend creates its schema at startup**: `AppState::new()` calls `schema::ensure_schema(&db)` right after connecting (`zapzap-rust/src/infrastructure/app_state.rs`), which runs `zapzap-rust/src/infrastructure/database/schema.sql` through `sqlx::raw_sql` **in one transaction** (`schema.rs`): a failing statement rolls back the ones before it and leaves the database as it was.
...
- `auth_middleware` requires `Authorization: Bearer <token>`, else bare 401 with empty body (`zapzap-rust/src/api/middleware/auth_middleware.rs`). `optional_auth_middleware` attaches claims if valid. `admin_middleware` is mounted on every `/api/admin` route, inside `auth_middleware` (`create_admin_router`, `zapzap-rust/src/api/routes/mod.rs`): it reads `is_admin` **from the database**, as Node's `adminMiddleware.js` does, and answers 401 `AUTH_REQUIRED` / 403 `ADMIN_REQUIRED` with `{success:false, error, code}`. A revoked admin is refused at once, although the token still says admin; the handlers also still check `claims.is_admin`, so a newly promoted admin needs a fresh login.
...
- **Per-user filtering** (`should_deliver`, `sse.rs`): an event without `partyId` (`userConnected`, `userDisconnected`, ...) goes to every client, as Node sends everything to everyone; a party event goes only to a client whose token names a player of that party (checked in `party_players` per event), or whose own action it reports (the player who just left), and `partyDeleted` also to the clients that were players of it when the stream opened or since (its membership rows are gone by then). A client without a valid token receives global events only. Node filters nothing (`src/api/server.js:69-151`).
