...
- `drl` and `ml` have **no Rust implementation**: they silently play as `hard` (the catch-all of `for_difficulty`). The DRL models in `data/models/` are used only by native training ([[NativeEngine]]).
...
- `VinceParams` defaults (`:81-128`) comment "optimized via genetic algorithm (74,000 games)"; they match `optimized.params` of `data/hard_vince_genetic_params.json` (goldenScoreJokerPenalty -393.857 → -393.86, highCardPairBreakingPenalty -168.99).
...
- `data/thibot_genetic_params.json`, `data/hard_vince_genetic_params.json`: outputs of genetic optimisations — Thibot's by `scripts/genetic-optimize-thibot.js:33` (uses the native engine, `:21`), Vince's by a JS optimiser removed with the Node backend. Keys: baseline, optimized, config, generationStats, timestamp.
...
- `data/models/*.safetensors` belong to native training ([[NativeEngine]]).
...
- Loop: stops on a human's turn or a finished round; a pause of `BOT_ACTION_DELAY_MS` after each bot action (1000 ms by default; production sets 2000). Eliminated bots are skipped by advancing `current_turn`. A failed discard draw falls back to the deck. Each action broadcasts a `gameUpdate` SSE with `isBot: true`.
