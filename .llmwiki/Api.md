...
- A JSON body that does not parse — malformed, a field missing or mistyped — answers **400** with the route's missing-field code (`ApiJson`, `zapzap-rust/src/api/error.rs`), not axum's 422 plain text. A request without a JSON content type reads as `{}`, as Express does.
...
|---|---|---|---|---|
| GET | `/` | opt | `zapzap-rust/src/api/routes/party.rs:344` | 500 `GET_PARTIES_ERROR`. Query `status`, `limit` (default 50, **no max**), `offset` (default 0) (`party.rs:354-355`). Only `visibility='public'` parties (`zapzap-rust/src/infrastructure/database/repositories/party_repo.rs:149`). Each item carries `isMyTurn`: true when the caller is a member, the party is playing, the round is not finished and `currentTurn` is the caller's seat (`zapzap-rust/src/application/party/list_parties.rs`); **Rust only**, Node has no such field |
| POST | `/` | JWT | `party.rs:276` | 400 `MISSING_PARTY_NAME`; 400 `VALIDATION_ERROR` for an unreadable body or a use-case validation error (Node: 500 `CREATE_PARTY_ERROR`); 500 `CREATE_PARTY_ERROR` on repository errors. Body `name`, `visibility` (default `public`), `settings{handSize=5,maxScore=100,enableGoldenScore=true,goldenScoreThreshold=100}` (`party.rs:286-294`), `botIds[]`. Emits `partyUpdate`/`partyCreated` with the new `partyId` (**Rust only**, Node emits nothing) |
| GET | `/:partyId` | JWT | `party.rs:385` | 404 `PARTY_NOT_FOUND`, 500. **No membership check**: any user gets details incl. `inviteCode` of private parties (`zapzap-rust/src/application/party/get_party_details.rs:44-100`) |
| DELETE | `/:partyId` | JWT | `party.rs:556` | 404, 403 `NOT_AUTHORIZED` (neither owner nor only human), 409 `PARTY_PLAYING` |
| POST | `/:partyId/join` | JWT | `party.rs:436` | 404, 409 `PARTY_FULL` (8 players, `zapzap-rust/src/domain/entities/party.rs:100-101`), 409 `ALREADY_IN_PARTY`, 409 `PARTY_STARTED` (not waiting). Body `inviteCode` is **ignored** by the use case |
| POST | `/:partyId/leave` | JWT | `party.rs:478` | 404, 403 `NOT_IN_PARTY`, 409 `PARTY_PLAYING` (not waiting). Owner leaving transfers ownership (`zapzap-rust/src/application/party/leave_party.rs:87-89`) |
| POST | `/:partyId/start` | JWT | `party.rs:513` | 404, 403 `NOT_OWNER`, 409 `PARTY_ALREADY_PLAYING` (not waiting), 400 `NOT_ENOUGH_PLAYERS` (3..=8 required, `zapzap-rust/src/domain/entities/party.rs:105-106`) |
| POST | `/:partyId/bots` | JWT | `party.rs:591` | **Rust only.** Body `{botId}`; the owner fills the lowest free seat of a waiting party with a bot (`zapzap-rust/src/application/party/add_bot_to_party.rs`). 201 `{success, party{id,name,status}, bot{id,username,botDifficulty}, playerIndex}`, and a `partyUpdate`/`playerJoined` event with the bot as `userId`. 400 `MISSING_BOT_ID`, 404 `PARTY_NOT_FOUND`, 403 `NOT_OWNER`, 409 `PARTY_STARTED`, 404 `BOT_NOT_FOUND`, 400 `NOT_A_BOT`, 409 `ALREADY_IN_PARTY`, 409 `PARTY_FULL` |
...
|---|---|---|---|---|
| GET | `/:partyId/state` | `zapzap-rust/src/api/routes/game.rs:293` | — | 404, 500. No membership check: a non-member gets an empty `playerHand` but all public state and, once a round is finished, `allHands`. Spawns a bot trigger (`game.rs:367`). `eliminatedPlayers` computed as score > 100 hard-coded (`game.rs:324-330`), ignoring `maxScore` |
| POST | `/:partyId/selectHandSize` | `game.rs:403` | `{handSize}` (4-7, 4-10 in golden score, `zapzap-rust/src/application/game/select_hand_size.rs`) | 400 `INVALID_ACTION_STATE` (checked first, as Node), 403 `NOT_YOUR_TURN`, 400 `INVALID_HAND_SIZE` (also for an unreadable body) |
| POST | `/:partyId/play` | `game.rs:450` | `{cardIds:[u8]}` | 400 `MISSING_CARDS` (empty, missing or unreadable), 400 `INVALID_ACTION_STATE`, 400 `INVALID_CARDS` (a card not in hand), 400 `INVALID_PLAY` (not a valid combination; Node: 500) |
| POST | `/:partyId/draw` | `game.rs:504` | `{source:"deck"\|"played", cardId?}`; no `cardId` from `played` takes the top card, as Node | 400 `INVALID_SOURCE` (also for an unreadable body), 400 `INVALID_ACTION_STATE`, 400 `DECK_EMPTY` (deck and discard empty; Node: 500), 400 `NO_CARDS_AVAILABLE`, 400 `CARD_NOT_AVAILABLE` |
| POST | `/:partyId/zapzap` | `game.rs:560` | — | 400 `INVALID_ACTION_STATE`, 400 `HAND_TOO_HIGH`; response below |
| POST | `/:partyId/nextRound` | `game.rs:661` | — | 404, 400 `INVALID_PARTY_STATE`/`ROUND_NOT_FINISHED`; **any authenticated user** can advance any party: `NextRound` never reads `input.user_id` and has no `NotInParty` error (`zapzap-rust/src/application/game/next_round.rs:238-246`) |
| POST | `/:partyId/trigger-bot` | `game.rs:742` | — | 500 on repo errors. Handler takes no `Claims`: any authenticated user can drive bots in any party |

#### zapzap response (Node's contract)
...
- `roundScores`: this round's points, keyed by player index for the active players — the key `/state` uses for the same values.
...
- `gameFinished: true` and `winner{userId, playerIndex, score}` only when the call ends the game.
...
| start | party finished | 500 | 409 `PARTY_ALREADY_PLAYING` |
