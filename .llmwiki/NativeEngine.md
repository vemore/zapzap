...
| Lint | `#![deny(clippy::all)]` at crate root, but CI runs no clippy on native | `native/src/lib.rs:5`, `.github/workflows/ci.yml:126` |
...
- `thibot` `test_zapzap_decision` is red and skipped in CI (`native/src/strategies/thibot.rs:919`, `.github/workflows/ci.yml:127`).

### Tests
98 `#[test]` functions across 19 modules (`grep -rn '#\[test\]' native/src`). Run `cd native && cargo test` (see [[Testing]]); CI runs `cargo fmt --check` and `cargo test -- --skip strategies::thibot::tests::test_zapzap_decision` without `--locked` (`.github/workflows/ci.yml:108-127`).
