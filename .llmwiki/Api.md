...
|---|---|---|---|---|
| GET | `/:partyId/state` | `zapzap-rust/src/api/routes/game.rs:294` | — | 404, 403 `NOT_IN_PARTY` for a non-member, as Node (`zapzap-rust/src/application/game/get_game_state.rs`), 500. Spawns a bot trigger (`game.rs:368`). `eliminatedPlayers` computed as score > 100 hard-coded (`game.rs:325-331`) |
...
| POST | `/:partyId/nextRound` | `game.rs:676` | — | 404, 403 `NOT_IN_PARTY` for a non-member (checked in the handler, `require_party_member`, `zapzap-rust/src/api/access.rs`; `NextRound` itself still never reads `input.user_id`), 400 `INVALID_PARTY_STATE`/`ROUND_NOT_FINISHED` |
