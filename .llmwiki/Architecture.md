...
- Production's `zapzap-backend` container runs the Rust binary from the root compose; the Rust compose names its own container `zapzap-rust-backend`. [[Deployment]].
