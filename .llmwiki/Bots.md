...
- No scheduler and no startup recovery (`zapzap-rust/src/main.rs:10-47`): bots move only when some request spawns `trigger_bot_internal` (`game.rs:1224`):
...
- Manual `POST /api/game/:partyId/trigger-bot` (auth required, no party-membership check; `zapzap-rust/src/api/routes/mod.rs:173-178`) runs the same logic synchronously, max 50 iterations, 100 ms pause (`game.rs:757-1220`).
