...
/// Whether a card id appears twice in `cards`: a play names each card once
pub fn has_repeated_card(cards: &[u8]) -> bool {
    cards
        .iter()
        .enumerate()
        .any(|(i, c)| cards[i + 1..].contains(c))
}
...
pub fn is_valid_play(cards: &[u8]) -> bool {
    match cards.len() {
        0 => false,
        1 => true,
        _ if has_repeated_card(cards) => false,
...
mod tests {
    ...
    fn test_can_call_zapzap() {
        assert!(!can_call_zapzap(&[0, 1, 2])); // 1+2+3 = 6 > 5
    ...
    fn test_is_valid_same_rank() {
        assert!(is_valid_same_rank(&[0, 13])); // Pair of aces
        assert!(is_valid_same_rank(&[0, 13, 26])); // Three aces
        assert!(!is_valid_same_rank(&[0, 1])); // Mixed ranks
        assert!(is_valid_same_rank(&[0, 52])); // Joker + card
    }
    ...
    fn test_repeated_card_is_no_valid_play() {
        ...
        assert!(has_repeated_card(&[5, 5]));
        assert!(has_repeated_card(&[0, 1, 0]));
        assert!(!has_repeated_card(&[0, 13, 26]));
