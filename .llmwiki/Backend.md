...
- `zapzap-rust/docker-compose.yml` sets `PORT=9999`, `DATABASE_URL=sqlite:/app/data/zapzap.db`, `JWT_SECRET=${JWT_SECRET:?...}` (no default: `docker compose` refuses to start without it), mounts `../data:/app/data`, healthcheck `curl -f http://localhost:9999/api/health`.
